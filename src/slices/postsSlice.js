...
export const getTrendingPosts = createAsyncThunk(
  ...
  async () => {
    return await axios
      .get("http://localhost:9000/api/posts/trending/posts")
...
export const getPostsByCategory = createAsyncThunk(
  ...
  async (filterByCategory) => {
    return await axios
      .get(
        `http://localhost:9000/api/posts/getPostsByCategory/${filterByCategory}`
      )
...
export const createPost = createAsyncThunk("posts/create", async (formData) => {
  ...
  return await axios
    .post("http://localhost:9000/api/posts/create", formData, {
      headers: {
...
export const updatePost = createAsyncThunk(
  ...
  async ({ id, formData }) => {
    ...
    return await axios
      .put(
        `https://theklinkers-blog-backend.onrender.com/api/posts/update/${id}`,
        formData,
        {
          headers: {
            Authorization: localStorage.getItem("token"),
...
export const updateLikePost = createAsyncThunk(
  ...
  async (id) => {
    ...
    return await axios
      .put(
        `https://theklinkers-blog-backend.onrender.com/api/posts/update/like/${id}`
      )
      .then((response) => response.data);
  }
);

export const updateDislikePost = createAsyncThunk(
  "posts/update/dislike",
  async (id) => {
    // console.log(`slice post ${id}`)
    // return await axios.put(`http://localhost:9000/api/posts/update/dislike/${id}`, { headers: {'Content-Type': 'application/json'}})
    return await axios
      .put(
        `https://theklinkers-blog-backend.onrender.com/api/posts/update/dislike/${id}`
      )
      .then((response) => response.data);
  }
);
...
export const deletePost = createAsyncThunk("posts/delete", async (id) => {
  ...
  return await axios
    .delete(
      `https://theklinkers-blog-backend.onrender.com/api/posts/delete/${id}`,
  ...
  reducers: {
    searchFilter: (state, action) => {
      ...
      state.posts = state.posts.filter((p) => {
        return p.postTitle === action.payload.searchInput;
      });
      state.error = "";
    },
    filterByCategory: (state, action) => {
      ...
      state.isLoading = false;
      state.posts = state.posts.filter((p) => {
        return p.postCategory === action.payload;
      });
      state.error = "";
    },
  },
