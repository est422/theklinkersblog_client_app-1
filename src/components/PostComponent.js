              ...
              <div key={i} className="col-lg-8 entries">
                <article className="entry entry-single">
                  <div className="entry-img">
                    <img
                      src={`https://theklinkers-blog-backend.onrender.com/images/${post.postImage}`}
                      alt=""
                      className="img-fluid"
                    />
                  ...
                  <div className="entry-meta">
                    <ul>
                      <li className="d-flex align-items-center">
                        ...
                        {new Date(post.postDate).toISOString().substring(0, 10)}
                      </li>
                      <li className="d-flex align-items-center">
                        <button
                          className="btn btn-none"
                          onClick={() => {
                            handleDislikeBtnClick(post.postId);
                          }}
                        >
                      ...
                      <li className="d-flex align-items-center">
                        <button
                          className="btn btn-none"
                          onClick={() => {
                            handleLikeBtnClick(post.postId);
                          }}
                        >
                      ...
                      {isLoggedIn ? (
                        <li className="d-flex align-items-center">
                          ...
                          <Link to={`/blogs/edit/${post.postId}`}>Edit</Link>
                        </li>
                      ) : null}
                      {isLoggedIn ? (
                        <li className="d-flex align-items-center">
                          <i className="bi bi-trash3"></i>
                          <Link to={`/blogs/delete/${post.postId}`}>
                            Delete
                          </Link>
                        </li>
                      ) : null}
                ...
                {trendingPosts.slice(0, 4).map((p, i) => (
                  <div key={i} className="sidebar-item recent-posts">
                    <div className="post-item clearfix">
                      ...
                      <img
                        src={`https://theklinkers-blog-backend.onrender.com/images/${p.postImage}`}
                        alt=""
                        className="img-fluid"
                      />
                      <h4>
